...
class Parameters:
    def __init__(self,
            ...
            smc_offset = 14,                      # number of weeks to delay SMC relative to last primary dose of first intervention
    ...
    def update_pars(self, update):
        ...
        if set(('all', 'min_vac_age', 'vac_age_range', 'study_months', 'ρ_elp', 'τ_elp')).intersection(update_set):
            ...
            assert self.τ_elp > 0, f'τ_elp must be positive, got {self.τ_elp}'
        ...
        if set(('all', 'min_vac_age', 'vac_age_range', 'study_months', 'ρ_clinical', 'δ_clinical', 'γ_clinical')).intersection(update_set):
            ...
            assert self.δ_clinical >= 0, f'δ_clinical must be non-negative, got {self.δ_clinical}'
            assert self.γ_clinical > 0, f'γ_clinical must be positive, got {self.γ_clinical}'
        ...
        if set(('all', 'min_vac_age', 'vac_age_range', 'study_months', 'lsv', 'vac_profile_liver', 'ν_liver', 'τ_liver', 'period_liver', 'nboosters_liver', 'nboosters')).intersection(update_set):
            ...
            assert self.ν_liver > 0, f'ν_liver must be positive, got {self.ν_liver}'
        ...
        if set(('all', 'offset_liver')).intersection(update_set):
            assert isinstance(self.offset_liver, int) and self.offset_liver >= 0, f'offset_liver must be non-negative integer, got {self.offset_liver}'

        if set(('all', 'offset_blood')).intersection(update_set):
            assert isinstance(self.offset_blood, int) and self.offset_blood >= 0, f'offset_blood must be non-negative integer, got {self.offset_blood}'
        ...
        if set(('all', 'smc_offset')).intersection(update_set):
            assert isinstance(self.smc_offset, int) and self.smc_offset >= 0, f'smc_offset must be non-negative integer, got {self.smc_offset}'
