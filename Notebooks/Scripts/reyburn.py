...
def pool_data():
    ...
    r = h_dead[0][:n] / (h_dead[0][:n]+h_alive[0][:n])
    return h_dead[1][:n], r

def plot_risk_pooled_data():
    h_dead, cfr = pool_data()
    fig, ax = plt.subplots(figsize=(4, 3))
    ax.scatter(h_dead, cfr)
    ax.set_xlabel('Age (years)')
    ax.set_ylabel('Severe malaria fatality risk')
...
def cfr_mean_CI(gam):
    """ from GAM calculate mean age-specific CFR and its 95% CI"""
    ...
    fit = f(confi)
    cfr = f(pdep)
    # CFR up to the maximum CFR
    idx = np.argmax(cfr)+1
    cfr_by_age = cfr[:idx]
    # CFR up to the maximum and add the last value so that CFR plateau
    dr = lambda x: np.interp(x, np.concatenate((XX[:idx, 0], [100])), list(cfr_by_age)+[cfr_by_age[-1]])

    return XX, fit, dr, cis

def plot_GAM_mean(cfr):
    fig, ax = plt.subplots()
    ...
    ax.set_ylabel('Severe malaria fatality risk', fontsize=18);
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)

    return fig, ax

def plot_GAM_mean_CI_data(XX, fit, cfr, cis, arrows=None):
    ...
    # Add the GAM fit
    ax.plot(XX[:, 0], cfr(XX[:, 0]), ls='--', lw=0.9, label='GAM fit')
...
def plot_GAM_mean_CI_data_infection(fit, cfr, cis, sm_age, λs):
    ...
    lines = colored_line(sm_age, cfr(sm_age), λs, ax, linewidth=6, cmap="jet",  norm=LogNorm(vmin=λs.min(), vmax=λs.max()))
    fig.colorbar(lines, format='%g', label='Blood infections per child per year')

    # Add the GAM fit
    ax.plot(x, cfr(x), ls='--', lw=0.9, label='GAM fit')
...
def plot_infection(cfr, sm_age, λs, filename, arrow=None):
    fig, ax = plt.subplots(1, 1, figsize=(7, 5))
...
def cfr_with_arrows():
    gam = fit_GAM()
    XX, fit, cfr, cis = cfr_mean_CI(gam)
    offset = 0.015
    a = (
        ( (0.25, cfr(0.25)+offset), (2.25, cfr(2.25)+offset) ),
        ( (0.25, cfr(0.25)+offset), (8, cfr(8)+offset) ),
        ( (5, cfr(5)+offset), (7, cfr(7)+offset) )
    )
    # plot_GAM_mean_CI_data(XX, fit, cfr, cis, 'reyburn_2015_mort_1_arrow.svg', arrows=a[:1])
    plot_GAM_mean_CI_data(XX, fit, cfr, cis, arrows=a)
