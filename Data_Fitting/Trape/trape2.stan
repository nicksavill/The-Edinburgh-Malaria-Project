// Individual-level sigmoidal non-hierarchical model of clinical malaria risk
...
model {
    ...
    for (i in 1 : T) {
        for (p in 1 : 6) {
            if (cases[i, p] >= 0) {
                real r = integrate_rate(periods[p], periods[p + 1], {lambda[i], rho_elp[i], tau_elp[i], delta_clinical[i], rho_clinical[i], gamma_clinical[i]});
...
generated quantities {
    ...
    for (i in 1 : T) {
        for (p in 1 : 6) {
            r[i, p] = integrate_rate(periods[p], periods[p + 1], {lambda[i], rho_elp[i], tau_elp[i], delta_clinical[i], rho_clinical[i], gamma_clinical[i]});
            if (cases[i, p] >= 0) {
