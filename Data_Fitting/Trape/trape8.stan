// Population-level sigmoidal model of clinical malaria risk homogeneous immunity parameters
...
parameters {
    real<lower=0> tau_elp; // half-life in years of early life protection against blood infection
    vector<lower=0>[2] lambda_raw; // rate of infection per child per year divided by 10
    real<lower=0> delta_clinical_raw; // decay rate of risk of clinical malaria per 100 infections
    real<lower=0> gamma_clinical_raw; // half-maximum number of infections divided by 100
    real<lower=0, upper=1> rho_elp; // protection from blood infection in newborns
    real<lower=0, upper=1> rho_clinical; // maximum proportion of infections that cause fever
}

transformed parameters {
    vector[2] lambda = lambda_raw * 10.; // scale to infections per year
    real delta_clinical = delta_clinical_raw / 100.; // scale to per infection
    real gamma_clinical = gamma_clinical_raw * 100.; // scale to infections
}
...
model {
    ...
    // likelihood
    for (i in 1 : N_dielmo) {
        real r = integrate_rate(dielmo_periods[i] * 52, dielmo_periods[i + 1] * 52, {lambda[1], rho_elp, tau_elp,
            delta_clinical, rho_clinical, gamma_clinical});
        dielmo_cases[i] ~ poisson(dielmo_person_years[i] * r);
    }
    for (i in 1 : N_ndiop) {
        real r = integrate_rate(ndiop_periods[i] * 52, ndiop_periods[i + 1] * 52, {lambda[2], rho_elp, tau_elp,
            delta_clinical, rho_clinical, gamma_clinical});
        ndiop[i] ~ poisson(M * r);
    }
}
