// Individual-level exponential hierarchical model of clinical malaria risk
...
model {
    ...
    for (i in 1 : T) {
        for (p in 1 : 6) {
            if (cases[i, p] >= 0) {
                real r = integrate_rate(periods[p], periods[p + 1], {lambda[i], 1.0, tau_elp[i], delta_clinical[i], rho_clinical[i]});
...
generated quantities {
    ...
    for (i in 1 : T) {
        for (p in 1 : 6) {
            r[i, p] = integrate_rate(periods[p], periods[p + 1], {lambda[i], 1.0, tau_elp[i], delta_clinical[i], rho_clinical[i]});
            if (cases[i, p] >= 0) {
